...
import "@/styles/components/header/userBtnList.scss";
...
export const UserBtnList: React.FC = () => {
    ...
    const baseUserBtnList = useRef<UserBtnType[]>([
        ...
        {id:"user-btn-4",title:"我的订单", link:"/modifyPassword"},
        {id:"user-btn-5",title:"我的钱包", link:"/wallet"},
    ...
    const btnHandler = (value:UserBtnType) => {
        if (value.id === "user-btn-7"){
            // 调用退出登录接口
            logout().then(r=>{
                if (r.code === 200){
                    dispatch(setMessageStatus({typeStatus:"success", message:"退出成功",description:r.msg}));
                                ...
                                <List>
                                    {
                                        baseUserBtnList.current.map(value=>(
                                            <List.Item key={value.id}>
                                                <div className={"list-item"} onClick={()=> btnHandler(value)}>
                                                    {value.title}
                                                </div>
                                            </List.Item>
                                        ))
